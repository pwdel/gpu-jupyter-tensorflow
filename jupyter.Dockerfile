...
# Install dependencies using the environment.yaml
COPY --chown=jovyan:users environment.yaml /tmp/environment.yaml
RUN conda env update -n tf_gpu_env -f /tmp/environment.yaml \
